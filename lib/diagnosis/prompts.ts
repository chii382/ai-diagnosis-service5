import { formatCareerPathPatternsForPrompt } from "@/lib/diagnosis/career-path-patterns";
import {
...
`
  ...
  const patternSection = formatCareerPathPatternsForPrompt();
...
## キャリアパスパターン（careerPathPatternId は必ずこの一覧から1つ選ぶ）
${patternSection}
...

{
  "result": {
    ...
    "careerPathPatternId": "上記一覧のIDから1つ（例: hybrid_product）",
  ...
  "resultBrief": {
    ...
    "careerPathPatternId": "result と同じID",
...
## careerPathPatternId の選び方（重要）
- 「ずばりあなたのキャリアパスは？」に表示される固定パターン。自由記述の careerPathHeadline は出力しない
- 診断全体（回答・summary・recommendedDirections）を踏まえ、最も近いパターンIDを1つ選ぶ
- result と resultBrief の careerPathPatternId は必ず同じにする
- recommendedDirections は個別の詳細アドバイスとして自由に書いてよい（パターンラベルとは別）
